...
const Page = () => {
  ...
  useEffect(() => {
    ...
    getBasket(basketId as string).then((val) => {
      ...
      setLoading(false);
      if (val != undefined)
        getUrl(val.image).then(setimage)
    });
  }, []);

  useEffect(() => {
    if (!basket || !basketId || loading) return;
    if (basket.sizes.length == 0) { delBasket(); }
    else
      updateBasket(basket, basketId as string);
  }, [basket, basketId]);
  ...
  return (<div className='min-h-screen bg-slate-200 p-10 ' >
    ...
    <div className='p-10 shadow-lg rounded-xl bg-white flex flex-row justify-between items-center'>
      ...
      <div className="">
        <button className='flex place-content-end justify-items-start p-2 mb-8'
          onClick={() => { setModalOpen(true) }}><BiSolidPencil size={24} /></button>
        <button className='flex place-content-end justify-items-end p-2 mt-8'
          onClick={() => { setdeleteBasketModal(true) }}><AiOutlineDelete size={24} /></button>
      </div>
    </div>
    <div>
      <div className="flex flex-wrap gap-[50px] p-10 justify-start mt-5 rounded-xl">
        ...
        <div className="grow p-5 bg-white rounded-xl shadow max-w-xs flex justify-center items-center opacity-40"
          onClick={() => { setSizeAdd(true) }}>
    ...
    {modalOpen && (
      <EditBasketModal
        basket={basket}
        onEdit={(bk: Basket) => { setModalOpen(false); setBasket(bk) }}
